...
## [0.32.0](https://github.com/worksonmyai/kontora/compare/v0.31.0...v0.32.0) - 2026-08-16

- update packages
- chore: use fictional placeholder names in examples and test data
- chore: remove private project names and paths from the repo
- refactor(cli): dispatch commands from a table and satisfy the linter
- docs: add a CLI reference covering every command and flag
- feat(cli): confirm ticket mutations and accept flags after positionals
- fix(cli): route the second prefix resolver through the shared ambiguity check
- feat(doctor): check worktrees dir, project repos and plannotator, and count warnings
- fix(cli): say --days is required and link the published config docs
- fix(config): redact the web token in kontora config output
- feat(cli): add stats, changes, activity, review and annotate commands
- fix(daemon): pass the daemon config path to agents via KONTORA_CONFIG
- fix(completion): generate the fish script from the command table
- fix(remote): name the unreachable daemon in transport errors
- fix(remote): explain why attach failed instead of dumping the handshake error
- fix(cli): print help to stdout and exit 0
- fix(remote): let fmt and completion run with KONTORA_URL set
- fix(remote): honour --closed and hide archived tickets in remote ls
- fix(api): keep the reason in invalid state transition errors
- feat(cli): add move command for human_review and custom statuses
- fix(cli): validate local status changes the way the daemon API does
- fix(init): apply --pipeline, --path and --agent in local mode
- fix(cli): reject ambiguous ticket ID prefixes instead of picking the first match
- Add per-stage reasoning effort with agent-level default
- Add clickable projects section to dashboard sidebar with project: and agent: filter tokens
- Go 1.26.6
