...
- **The component is a set of mixins.** Each `ui/*.js` exports a factory returning part of one flat Alpine object; `ui/index.js` merges them and assigns `kontora`, `termState` and `statsDerive` to `globalThis`. No two mixins may define the same key — `merge()` throws on a repeat, and `testdata/ui_mixins.test.mjs` names both owners. A key repeated inside one mixin is an esbuild warning, which fails the build. The assistant pane is `ui/assistant.js`; every key it defines is prefixed `assistant*` or `_assistant*`, because `ui/activity.js` already owns the unprefixed tool-row helpers (`toolKey`, `toolExpanded`, `toolFailed`, `toggleTool`, `eventTime`) for the ticket transcript.
