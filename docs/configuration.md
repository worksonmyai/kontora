...
| `checkpoint_compaction_tokens` | no | Enables phase-boundary compaction for `pi` when positive. The extension compacts only when measured context tokens are greater than this value. Zero or unset disables the checkpoint tool. Negative values and use on a non-`pi` agent fail validation. Wrapped `pi` agents, such as `nono run -- pi`, are supported. |

When checkpoint compaction is enabled, the prompt requires a durable `phase-N:` ticket note before each `kontora_phase_complete` call. The note records changed files, decisions, test results, unresolved issues, and the next phase. Pi records skipped, requested, compacted, and failed checkpoints in the session. A compaction failure does not fail the stage. Pi continues the next phase from the uncompacted session.

A config reload applies a changed threshold to the next agent invocation. It does not change a running Pi process. The web Settings form preserves `checkpoint_compaction_tokens` when it edits another field, but does not expose a control for it.
