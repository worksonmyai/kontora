	...
	tests := []struct {
		...
		},
		{
			...
			tickets: []Ticket{
				...
				{ID: "b", Status: "done", Path: "/elsewhere/astra-l", StartedAt: ago(4, 9), CompletedAt: ago(3, 9)},
				{ID: "c", Status: "done", Path: "/elsewhere/astra-l", StartedAt: ago(4, 9), CompletedAt: ago(3, 9)},
			},
			opts: Options{Now: testNow, Days: 35},
			check: func(t *testing.T, r Result) {
				require.Len(t, r.Projects, 2)
				require.Equal(t, "astra-l", r.Projects[0].Name)
		...
		},
		{
			...
			opts: Options{Now: testNow, Days: 35, Project: "astra-l"},
			check: func(t *testing.T, r Result) {
				require.Equal(t, 1, r.Totals.Shipped)
